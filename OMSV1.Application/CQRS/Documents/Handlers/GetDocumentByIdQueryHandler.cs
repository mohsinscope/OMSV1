...
namespace OMSV1.Application.Queries.Documents
{
        ...
        : IRequestHandler<GetDocumentByIdDetailedQuery, DocumentDetailedDto>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var totalCount = await GetTotalRelatedDocumentsCount(request.Id, cancellationToken);

            // 2) Gather IDs up to requested `Depth`
            var toLoad = new List<Guid> { request.Id };
            ...
            var root = docs.Single(d => d.Id == request.Id);

...
namespace OMSV1.Application.Handlers.Governorates
{
    public class UpdateGovernorateCommandHandler : IRequestHandler<UpdateGovernorateCommand, bool>
    {
        ...
        public UpdateGovernorateCommandHandler(IGenericRepository<Governorate> repository)
        {
        ...
        public async Task<bool> Handle(UpdateGovernorateCommand request, CancellationToken cancellationToken)
        {
            ...
            // Use constructor properties to enforce immutability
            var updatedGovernorate = new Governorate(request.Name, request.Code);
            await _repository.UpdateAsync(updatedGovernorate);
            return true;
        }
    }
}
